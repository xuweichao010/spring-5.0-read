...
1.1.3 创建一个 ClassPathBeanDefinitionScanner#ClassPathBeanDefinitionScanner(BeanDefinitionRegistry) bean的定义扫描器（真正的bean的扫描不在这里）
...
1.2.1.1.1.7 应用Scope代理模式（Spring MVC使用）
1.2.1.1.1.8 BeanDefinitionReaderUtils#registerBeanDefinition(BeanDefinitionHolder, BeanDefinitionRegistry) 调用工具类注册bean的定义信息。
...
1.3.5.3.2.1.1 定义一个 configCandidates 数组 获取BeanDefinitionRegistry中所有的bean的定义的名字
1.3.5.3.2.1.2 遍历bean定义的名字，通过名字取出BeanDefinition 信息 判断这个定义中是否有Full和Lite标记如果有证明这个类已经被处理过就不处理，如果没有标记继续判断这个类是否是一个配置类
	主要通过类上面是否加有 @Component、@ComponentScan、@Import、@ImportResource 这些注解。 如果是则把这个类的定义添加到 configCandidates 数组中。
1.3.5.3.2.1.3 判断 configCandidates 数组是否为空，如果为空则退出，不为空对 configCandidates 数组进行排序，实例化ConfigurationClassParser解析配置类对象。
1.3.5.3.2.1.4 定义candidates Set数组并对 configCandidates 去重
1.3.5.3.2.1.5 调用 ConfigurationClassParser#parse(Set) 方法解析配置对象。
1.3.5.3.2.1.5.1 定义一个延时加载的 deferredImportSelectors 数组 用来处理延时加载的 ImportSelector
1.3.5.3.2.1.5.2.遍历Set数组 如果 AnnotatedBeanDefinition 对象
1.3.5.3.2.1.5.2.1 调用 ConfigurationClassParser#parse(AnnotationMetadata, String) 方法 只是个空壳方法
1.3.5.3.2.1.5.2.1.1 ConfigurationClassParser#processConfigurationClass(ConfigurationClass)
1.3.5.3.2.1.5.2.1.1.1 检测是否要跳过解析和检测这个类是否是被其他的类导入到了，如果没有则跳过处理
1.3.5.3.2.1.5.2.1.1.2 调用ConfigurationClassParser#doProcessConfigurationClass(ConfigurationClass, SourceClass)方法处理配置对象
1.3.5.3.2.1.5.2.1.1.2.1 处理内部类和配置类上面是否有@PropertySources注解
1.3.5.3.2.1.5.2.1.1.2.2 处理@ComponentScans 注解 Spring去扫描注解从这里开始  //TODO 后面详细写，这里才是Spring真正的取扫描Bean
1.3.5.3.2.1.5.2.1.1.2.3 处理@Import注解@Import的三种情况，第一种直接导入一个类，导入一个实现了importSelector的类，或者实现了 ImportBeanDefinitionRegistrar的实现类
1.3.5.3.2.1.6 重新加载新的BeanDefinitions,因为在扫描的过程中，可能存在一些特殊的类，比如：ImportBeanDefinitionRegistrar那么也在这个方法里面处理，但是并不是包含在configClasses当中，
	configClasses当中主要包含的是importSelector,因为ImportBeanDefinitionRegistrar在扫描出来的时候已经被添加到一个list当中去了.
