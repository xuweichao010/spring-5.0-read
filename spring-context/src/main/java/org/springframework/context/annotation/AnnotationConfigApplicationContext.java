...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		 ...
		 * AnnotationConfigApplicationContext一定会去实现这个接口 BeanDefinitionRegistry
		 * AnnotationConfigApplicationContext <-e- GenericApplicationContext <-i- BeanDefinitionRegistry
		 * 在这个地方BeanDefinitionRegistry有什么用？ 通过这个接口的名称可以知道 他是BeanDefinition注册类（具体描述可以参考接口描述）
		 *
		 */
	 ...
	 */
	/**
	 * BeanDefinitionRegistry 注册我们Spring的配置类 即向注册器中添加了一个TestSpringd对象
	 * 这里的register有两个意思 第一个是可以注册Spring的配置类还可以单独注册一个Bean
	 */
	public void register(Class<?>... annotatedClasses) {
		Assert.notEmpty(annotatedClasses, "At least one annotated class must be specified");
		//调用读取器 注册annotatedClasses 读取器是 AnnotatedBeanDefinitionReader 这个对象
